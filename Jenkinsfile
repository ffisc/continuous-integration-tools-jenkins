  ...
  stages {
    ...
    stage('Deploy Staging') {
      steps {
        sh '''
                    #!/bin/bash
                    make deploy                         PLATFORM="Jenkins"                         FUNCTION=${STAGING_FUNCTION_NAME}                         VERSION=${GIT_COMMIT}                         BUILD_NUMBER=${BUILD_NUMBER}
                '''
      }
    }
    ...
    stage('Deploy Production') {
      steps {
        sh '''
                    #!/bin/bash
                    make deploy                         PLATFORM="Jenkins"                         FUNCTION=${PRODUCTION_FUNCTION_NAME}                         VERSION=${GIT_COMMIT}                         BUILD_NUMBER=${BUILD_NUMBER}
                '''
      }
    }
